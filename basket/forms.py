...
from core_forms import widgets
...
class AddToBasketForm(forms.Form):
    ...
    def clean(self):
        stock_chado = StockRecord.objects.filter(product=self.product, partner__code='chadomarket').first()
        if stock_chado:
            if stock_chado.num_in_stock > stock_chado.num_allocated:
                info = self.basket.strategy.fetch_for_product(self.product, stock_chado)
            else:
                stock_rec = StockRecord.objects.filter(product=self.product).exclude(partner__code='chadomarket').first()
                info = self.basket.strategy.fetch_for_product(self.product, stock_rec)
        else:
            info = self.basket.strategy.fetch_for_product(self.product)

        for b in self.basket.lines.all():
            print('self.basket.lines.all() ===== ', b.product)
        ...
        actual_stocks = self.product.num_in_stock - self.product.num_allocated
        if self.cleaned_data.get('quantity', 1) > actual_stocks:
            raise forms.ValidationError("a maximum of {} can be bought".format(actual_stocks))
