...
def get_lines_in_basket(request, *args, **kwargs):
    lines = []
    for l in request.basket.lines.all():
        size = ProductAttributeValue.objects.filter(attribute__code='size', product=l.product).first()
        color = ProductAttributeValue.objects.filter(attribute__code='color1', product=l.product).first()

        line = {
            'title': l.product.title,
            'artikul': l.product.artikul,
            'color': color.value_option if color else None,
            'size': size.value_option if size else None,
            'quantity': l.quantity,
            'unit_price_with_discount': l.unit_price_with_discount,
            'price_incl_tax': l.price_incl_tax,
