...
from partner import availability
from core.compat import AUTH_USER_MODEL
from core.utils import get_default_currency
from common.templatetags.currency_filters import currency
...
class Basket(models.Model):
    ...
    def __str__(self):
        return _(
            u"%(status)s basket (owner: %(owner)s, lines: %(num_lines)d, pk: %(pk)d)") \
               % {'status': self.status,
                  'owner': self.owner,
    ...
    def add_product(self, product, quantity=1, options=None):
        ...
        if created:
            ...
            line.quantity = max(0, line.quantity + int(quantity))
            line.save()

        # Ставим в резерв
        product.num_allocated += int(quantity)
        product.save()
        stockrecord = product.stockrecords.first()
        stockrecord.num_allocated += int(quantity)
        stockrecord.save()
        print("Сложение произошло")
        StockReserveModel = get_model('basket', 'StockReserve')
        stock_reserve = StockReserveModel.objects.filter(line=line).first()
        lines_ids = get_model('basket', 'Line').objects.values_list('id', flat=True).filter(basket=self)
        start_time = timezone.now()
        end_time = start_time + timezone.timedelta(minutes=30)
        if stock_reserve:
            stock_reserve.quantity = line.quantity
            stock_reserve.start_reserve = start_time
            stock_reserve.end_reserve = end_time
            stock_reserve.save()
        else:
            stock_reserve = StockReserveModel()
            stock_reserve.line = line
            stock_reserve.start_reserve = start_time
            stock_reserve.end_reserve = end_time
            stock_reserve.save()

        StockReserveModel.objects.filter(line_id__in=lines_ids).update(start_reserve=start_time,
                                                                       end_reserve=end_time)
    ...
    def get_total_sum_without_discount(self):
        total = D('0.00')
        for l in self.lines.all():
            print(type(l.price_excl_tax))
            total += l.price_excl_tax
        print("total", total)
        return total

    @property
    def get_total_sum_with_discount(self):
        total = D('0.00')
        for l in self.lines.all():
            total += l.price_excl_tax
        return total
...
class Line(models.Model):
    ...
    @property
    def line_price_with_discount(self):
        discount = self.product.discounts.filter(start_datetime__lte=timezone.now(),
                                                 end_datetime__gte=timezone.now()).first()
        total_sum = self.unit_price_with_discount * self.quantity
        return total_sum
    ...
    def unit_price_with_discount(self):
        discount = self.product.discounts.filter(start_datetime__lte=timezone.now(),
                                                 end_datetime__gte=timezone.now()).first()
        if discount:
            return math.ceil(self.unit_price_excl_tax - (discount.discount_value * self.unit_price_excl_tax) / 100)
...
class OldReserveLine(models.Model):
    basket = models.ForeignKey('basket.Basket', related_name='old_lines')
    product = models.ForeignKey(
        'catalogue.Product', related_name='old_reserve_lines')
    ...
    def unit_price_with_discount(self):
        discount = self.product.discounts.filter(start_datetime__lte=timezone.now(),
                                                 end_datetime__gte=timezone.now()).first()
        if discount:
            return math.ceil(self.product.price_mrc - (discount.discount_value * self.product.price_mrc) / 100)
        return self.product.price_mrc
...
class LineAttribute(models.Model):
    ...
    class Meta:
        app_label = 'basket'
        verbose_name = 'Атрибут строки'
        verbose_name_plural = 'Атрибуты строки'


class DeliveryBasket(models.Model):
    basket = models.ForeignKey(Basket)
    post_id = models.ForeignKey(Office, null=True, blank=True)
    post_id_pick = models.ForeignKey(OfficePickPoint, null=True, blank=True)
    ...
    delivery_time_interval = models.CharField(max_length=50, null=True, blank=True, choices=Order.INTERVALS_VARIANTS)
...
class StockReserve(models.Model):
    line = models.ForeignKey(Line, related_name='stock_reserve_lines', null=True, blank=True)
    start_reserve = models.DateTimeField('Начало резервирования', default=timezone.now)
    end_reserve = models.DateTimeField('Конец резервирования', default=timezone.now)
...
class BasketHistoryReserve(models.Model):
    basket = models.ForeignKey(Basket)
    ...
    class Meta:
        verbose_name = 'Корзина(История резерва)'
        verbose_name_plural = 'Корзина(История резерва)'
